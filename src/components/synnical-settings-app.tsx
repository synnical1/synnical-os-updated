...
import { AccountRoleStatus } from "./account-role-status"
...
export function SynnicalSettingsApp() {
  ...
  useEffect(() => { hydrateOsSettings().then((next) => { setOs(next); setLoaded(true) }) }, [user?.id])
  useEffect(() => {
  ...
  const personalization = <div className="space-y-2">
    <SectionHeader title="Appearance" subtitle="Light or dark, accents, backgrounds and desktop." /><AppearanceModeControl />
    <SettingCard icon={MousePointer2} title="Cursor" desc="In-app pointer style and size."><div className="mt-2 flex gap-3"><select aria-label="Cursor theme" value={os.cursorTheme} onChange={(e) => void patchOs("cursorTheme", e.target.value as OsSettings["cursorTheme"])} className="bg-[#111]">{["system", "light", "dark", "crosshair"].map((value) => <option key={value}>{value}</option>)}</select><input autoComplete="off" name="preferences-synnical-settings-app-1" aria-label="Cursor size" type="range" min={75} max={175} step={5} value={os.cursorSize} onChange={(e) => void patchOs("cursorSize", Number(e.target.value))} /><span>{os.cursorSize}%</span></div></SettingCard>
    ...
    <SettingCard icon={Sparkles} title="Visual effects" desc="Synnical OS animation, liquid glass and wallpaper readability controls."><div className="mt-3 grid gap-3 sm:grid-cols-2">
      ...
      <label className="text-xs text-white/60">Glass strength <span className="text-white/35">{os.glassStrength}%</span><input autoComplete="off" name="preferences-synnical-settings-app-2" className="mt-1 w-full" type="range" min="20" max="100" value={os.glassStrength} onChange={(e)=>patchOs("glassStrength",Number(e.target.value))} /></label>
      <label className="text-xs text-white/60">Wallpaper dim <span className="text-white/35">{os.wallpaperDim}%</span><input autoComplete="off" name="preferences-synnical-settings-app-3" className="mt-1 w-full" type="range" min="0" max="75" value={os.wallpaperDim} onChange={(e)=>patchOs("wallpaperDim",Number(e.target.value))} /></label>
      <label className="text-xs text-white/60">Wallpaper blur <span className="text-white/35">{os.wallpaperBlur}px</span><input autoComplete="off" name="preferences-synnical-settings-app-4" className="mt-1 w-full" type="range" min="0" max="24" value={os.wallpaperBlur} onChange={(e)=>patchOs("wallpaperBlur",Number(e.target.value))} /></label>
      <label className="text-xs text-white/60">Wallpaper saturation <span className="text-white/35">{os.wallpaperSaturation}%</span><input autoComplete="off" name="preferences-synnical-settings-app-5" className="mt-1 w-full" type="range" min="0" max="180" value={os.wallpaperSaturation} onChange={(e)=>patchOs("wallpaperSaturation",Number(e.target.value))} /></label>
    </div></SettingCard>
  ...
  const accounts = <div className="space-y-2"><SectionHeader title="Accounts" subtitle="Your profile, sign-in, sessions and linked Synnical identity." /><AccountRoleStatus />
    ...
    <SettingCard icon={Lock} title="Change password" desc="Confirm your current password, choose a new one, and optionally sign out other devices."><form id="account-credential-change" autoComplete="on" onSubmit={(e) => { e.preventDefault(); void changePassword() }} className="mt-3 grid gap-2"><input name="account-current-secret" aria-label="Current password" type="password" autoComplete="current-password" value={currentPassword} onChange={(e)=>setCurrentPassword(e.target.value)} placeholder="Current password" className="rounded-lg border border-white/10 bg-black/25 px-3 py-2 text-xs outline-none" /><input type="password" autoComplete="new-password" value={newPassword} onChange={(e)=>setNewPassword(e.target.value)} placeholder="New password" className="rounded-lg border border-white/10 bg-black/25 px-3 py-2 text-xs outline-none" /><input type="password" autoComplete="new-password" value={confirmPassword} onChange={(e)=>setConfirmPassword(e.target.value)} placeholder="Confirm new password" className="rounded-lg border border-white/10 bg-black/25 px-3 py-2 text-xs outline-none" /><label className="flex items-center justify-between gap-3 text-xs text-white/60">Sign out other devices<Toggle value={revokeOthers} onChange={setRevokeOthers} label="Sign out other devices after password change" /></label><button disabled={passwordBusy || !currentPassword || !newPassword || !confirmPassword} type="submit" className="rounded-lg bg-sky-500 px-3 py-2 text-xs font-semibold text-black disabled:opacity-40">{passwordBusy?"Changing password…":"Change password"}</button></form></SettingCard>
  ...
  return <div className="synnical-settings-app flex h-full min-h-0 bg-[#0c0c0e] text-white">
    <aside className="w-[250px] shrink-0 overflow-y-auto border-r border-white/10 bg-white/[0.02] p-3 custom-scroll">
      ...
      <div className="mb-3 flex h-9 items-center rounded-lg border border-white/10 bg-black/25 px-2"><Search className="h-3.5 w-3.5 text-white/35" /><input type="search" id="settings-filter" name="settings-filter" autoComplete="off" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Find a setting" className="min-w-0 flex-1 bg-transparent px-2 text-xs outline-none" /></div>
