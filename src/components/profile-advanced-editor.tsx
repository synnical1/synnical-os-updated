    ...
    <div className="mt-6 space-y-4">
      <div className="rounded-xl border border-[var(--synnical-border)] bg-[var(--synnical-surface-2)] p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label="Birthday"><Input autoComplete="off" name="preferences-profile-advanced-editor-1" type="date" value={birthday} onChange={(e) => setBirthday(e.target.value)} /></Field>
          ...
          <Field label="Pronouns"><Input autoComplete="off" name="preferences-profile-advanced-editor-2" maxLength={40} value={pronouns} onChange={(e) => setPronouns(e.target.value)} placeholder="Optional" /></Field>
          ...
          <Field label="Status expires"><Input autoComplete="off" name="preferences-profile-advanced-editor-3" type="datetime-local" value={statusExpiry} onChange={(e) => setStatusExpiry(e.target.value)} /></Field>
        </div>
      ...
      <div className="rounded-xl border border-[var(--synnical-border)] bg-[var(--synnical-surface-2)] p-4">
        ...
        <Field label="Accent gradient"><Input autoComplete="off" name="preferences-profile-advanced-editor-4" value={gradient} onChange={(e) => setGradient(e.target.value)} placeholder="linear-gradient(135deg, #001a44, #000000)" /></Field>
        <div className="mt-3 grid gap-3 sm:grid-cols-2"><Field label={`Banner horizontal position · ${bannerX}%`}><Input autoComplete="off" name="preferences-profile-advanced-editor-5" type="range" min={0} max={100} value={bannerX} onChange={(e) => setBannerX(Number(e.target.value))} /></Field><Field label={`Banner vertical position · ${bannerY}%`}><Input autoComplete="off" name="preferences-profile-advanced-editor-6" type="range" min={0} max={100} value={bannerY} onChange={(e) => setBannerY(Number(e.target.value))} /></Field></div>
        <div className="mt-3 grid gap-3 sm:grid-cols-2"><Field label="Profile music provider"><NativeSelect value={musicProvider} onChange={setMusicProvider} options={["audius", "piped", "invidious"]} /></Field><Field label="Track ID"><Input autoComplete="off" name="preferences-profile-advanced-editor-7" value={musicTrackId} onChange={(e) => setMusicTrackId(e.target.value)} placeholder="Provider track/video ID" /></Field><Field label="Profile music title"><Input autoComplete="off" name="preferences-profile-advanced-editor-8" value={musicTitle} onChange={(e) => setMusicTitle(e.target.value)} /></Field><Field label="Artist"><Input autoComplete="off" name="preferences-profile-advanced-editor-9" value={musicArtist} onChange={(e) => setMusicArtist(e.target.value)} /></Field><Field label="Artwork URL"><Input autoComplete="off" name="preferences-profile-advanced-editor-10" value={musicArtwork} onChange={(e) => setMusicArtwork(e.target.value)} /></Field></div>
      ...
      <div className="rounded-xl border border-[var(--synnical-border)] bg-[var(--synnical-surface-2)] p-4">
        ...
        <div className="flex flex-col gap-2 sm:flex-row"><Input autoComplete="off" name="preferences-profile-advanced-editor-11" value={linkLabel} onChange={(e) => setLinkLabel(e.target.value)} placeholder="Label" className="sm:w-40" /><Input autoComplete="off" name="preferences-profile-advanced-editor-12" value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} placeholder="https://example.com" /><Button size="sm" onClick={() => void act("add-link", { action: "add-link", label: linkLabel, url: linkUrl }, "Link added")} disabled={!linkLabel || !linkUrl || busy === "add-link"}>Add</Button></div>
      ...
      <div className="rounded-xl border border-[var(--synnical-border)] bg-[var(--synnical-surface-2)] p-4">
        ...
        <div className="grid gap-2 sm:grid-cols-[140px_1fr_1fr_auto]"><NativeSelect value={showcaseKind} onChange={setShowcaseKind} options={["game", "achievement", "badge", "cosmetic"]} /><Input autoComplete="off" name="preferences-profile-advanced-editor-13" value={showcaseRef} onChange={(e) => setShowcaseRef(e.target.value)} placeholder="Reference ID" /><Input autoComplete="off" name="preferences-profile-advanced-editor-14" value={showcaseLabel} onChange={(e) => setShowcaseLabel(e.target.value)} placeholder="Display label" /><Button size="sm" onClick={() => void act("showcase", { action: "showcase", kind: showcaseKind, refId: showcaseRef, label: showcaseLabel }, "Showcase saved")}>Add</Button></div>
