...
export function R7SecurityControls() {
  ...
  return <div className="space-y-5">
    ...
    <div className="rounded-xl border border-[var(--synnical-border)] p-4"><div className="flex items-center justify-between gap-3"><div><h3 className="font-semibold">Password confirmation</h3><p className="text-xs text-[var(--synnical-muted)]">Sensitive security changes require your password again.</p></div><Input name="account-security-confirmation" value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="Current password" className="max-w-xs bg-black" autoComplete="current-password" /></div></div>
    ...
    <div className="rounded-xl border border-[var(--synnical-border)] p-4 space-y-3"><h3 className="font-semibold">Recovery question</h3><p className="text-xs text-[var(--synnical-muted)]">Confirm your account password above to replace your recovery question and answer.</p><Input autoComplete="off" name="preferences-r7-settings-2" aria-label="Recovery question" maxLength={180} value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="New recovery question" /><Input aria-label="Recovery answer" type="password" maxLength={220} value={answer} onChange={(e) => setAnswer(e.target.value)} placeholder="Recovery answer" autoComplete="off" /><Button disabled={busy || question.trim().length < 8 || answer.trim().length < 3} onClick={() => void protectedAction({ action: "change-security-question", securityQuestion: question, securityAnswer: answer }).then(() => setAnswer(""))}>Save recovery question</Button></div>

    <div className="rounded-xl border border-[var(--synnical-border)] p-4 space-y-3"><h3 className="font-semibold">Optional lock-screen PIN</h3><p className="text-xs text-[var(--synnical-muted)]">Enter your account password first. A PIN only unlocks an existing signed-in session; sign-in still requires your password.</p><Input aria-label="New lock-screen PIN" type="password" inputMode="numeric" maxLength={8} value={pin} onChange={(e) => setPin(e.target.value.replace(/\D/g, ""))} autoComplete="new-password" placeholder="4–8 digits" /><div className="flex gap-2"><Button disabled={busy || !/^\d{4,8}$/.test(pin)} onClick={() => void protectedAction({ action: "set-lock-pin", pin }).then(() => setPin(""))}>{state.pinConfigured ? "Change PIN" : "Set PIN"}</Button>{state.pinConfigured && <Button variant="outline" disabled={busy} onClick={() => void protectedAction({ action: "remove-lock-pin" })}>Remove PIN</Button>}</div></div>
